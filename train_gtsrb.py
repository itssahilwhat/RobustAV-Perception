...
def train_model(model, train_loader, val_loader, num_epochs=5, device='cuda', checkpoint_path=None, adv_training=False):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for images, labels in train_loader:
            ...
            # Optionally generate adversarial examples and mix them with clean images
            if adv_training:
                adv_images = fgsm_attack(model, images, labels)
                images = 0.5 * images + 0.5 * adv_images
...
def main():
    ...
    trained_model = train_model(model, train_loader, val_loader, args.epochs, args.device, checkpoint_path, adv_training=args.adv_training)
